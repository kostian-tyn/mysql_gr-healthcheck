...
:file:`/etc/exabgp/exabgp.conf`::
    ...
    process watch-mysql {
       run python healthcheck_mysql.py --config /some/file.cfg --label mysql;
    }
...
!!!
The program has been modified to check MySQL Group Replication Primary/Secondary member availability (by Kostiantyn Tertychnyi)
Now it makes persistent connection to database and uses SQL query instead of arbitrary command executing:
'SELECT viable_candidate, read_only, transactions_behind FROM sys.gr_member_routing_candidate_status'
You have to add the view to DB previously - https://gist.github.com/lefred/6f79fd02d333851b8d18f52716f04d91
The script checks primary member by default. If you need to check secondary one, put "readonly" option to configuration file.
"""
...
logger = logging.getLogger("healthcheck_mysql")
...
try:
    ...
    sys.stderr.write(
            '\n'
            'This program requires the python module mysql.connector\n'
            'Please pip install one of them with one of the following command.\n'
...
def parse():
    ...
    g.add_argument("--dbhost", metavar="IP or FQDN",
                   help="The host name or IP address of the MySQL server")
    g.add_argument("--dbport", metavar="PORT",
                   default=3306,
                   type=int,
                   help="The TCP/IP port of the MySQL server")
    g.add_argument("--dbuser", metavar="USER",
                   help="database username")
    g.add_argument("--dbpassword", metavar="PASSWORD",
                   help="database password")
    g.add_argument("--auth-plugin", metavar="NAME",
                   default="mysql_native_password",
                   dest="auth_plugin",
                   type=str,
                   help="MySQL authentication plugin to use: mysql_native_password (by default) or sha256_password")
    g.add_argument("--max-queue", metavar='COUNT',
                   default=100,
                   type=int, dest="max_queue",
                   help="The number of transactions that this member has received from the replication group which are waiting to be applied")
    g.add_argument("--readonly", action="store_true",
                   default=False,
                   help="enable SECONDARY member checking")
...
def setup_logging(debug, silent, name, syslog_facility, syslog):
    ...
    def syslog_address():
        ...
        if sys.platform == "darwin":
            return str("/var/run/syslog")
        if sys.platform.startswith("freebsd"):
            return str("/var/run/log")
        if sys.platform.startswith("netbsd"):
            return str("/var/run/log")
        if sys.platform.startswith("linux"):
            return str("/dev/log")
        if sys.platform.startswith("sunos"):
            return tuple(('127.0.0.1', 514))
...
def check_ips(ips):
    ...
    def which_ifconfig():
      ...
      raise EnvironmentError("Unable to guess ifconfig location for your platform")
    
    ifconfig = which_ifconfig()
    if os.path.exists(ifconfig):
      ...
      for address in ips:
        ...
        for ip in net:
          ip = str(ip)
          match = re.search(ip,out)
          if match is None:
            raise EnvironmentError('IP is not assigned at any interface: {0}'.format(ip))
    else:
      raise EnvironmentError("ifconfig does not exist")
...
def loop(options):
    ...
    def exabgp(target):
        ...
        logger.debug("send announces for %s state to ExaBGP", target)
    ...
    def trigger(target):
        ...
        logger.info("Transition to %s", str(target))
        cmds = []
    ...
    def connect_to_db():
      try:
        ...
        codes_to_retry = [2003,2006,2013]
        if err.errno in codes_to_retry:
          pass
        else:
          raise
      else:
    ...
    def check_db(connection):
      ...
      #logger.debug("Checking database: %s", repr(query))
      ...
      if (connection.is_connected()):
        try:
          ...
          logger.warn("{0}".format(err))
          return False
        if row is not None:
          online, readonly, queue = row
          logger.debug("Online: {0}; Readonly: {1}; Queue: {2}; Looking for SECONDARY: {3}".format(online,readonly,queue,options.readonly))
          if  online == 'YES' and readonly == 'NO' and queue <= options.max_queue and options.readonly == False:
            logger.debug("The node {0} is PRIMARY".format(options.dbhost))
            return True
          elif online == 'YES' and readonly == 'YES' and queue <= options.max_queue and options.readonly == True:
            logger.debug("The node {0} is SECONDARY".format(options.dbhost))
            return True
          else:
            logger.debug("The node is not online or lagging")
            return False
        else:
          logger.warn("gr_member_routing_candidate_status view returns None")
          return False
      else:
    ...
    state = states.INIT
    connection = None
    logger.info('Connecting to {0}'.format(options.dbhost))
    while connection is None:
        connection = connect_to_db()
        time.sleep(1)
    while True:
        if connection is None:
          #logger.info("Reconnecting to {0}".format(options.dbhost))
          connection = connect_to_db()
        else:
          if not connection.is_connected():
            #logger.info("Restoring the connection to {0}".format(options.dbhost))
            connection = connect_to_db()
        checks, state = one(checks, state, connection)
...
def main():
    ...
    try:
        ...
        # Check if IP addresses we're going to announce are assigned at any interface        
        check_ips(options.ips)
        # Main loop
        loop(options)
